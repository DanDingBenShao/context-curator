...
### Step 0: 审视需求
审视本轮用户消息和上下文, 判断用户的关注焦点是否发生变化。如果变化了, 更新记忆管理策略。
策略是给后续评分的宏观指引, 例如: "当前聚焦数据库连接池调优, 日志分析段落只在网络相关时有参考价值, Redis 相关内容暂不需要"。
同时检查用户是否明确要求"记住"/"不要忘记"/"这条很重要"等——如有, 将对应段落的 id 加入 `pinned_segments`。
输出字段: `strategy_update` (null 或新策略文本), `pinned_segments`
...
### Step 3: 知识缺口
判断是否需要补充外部信息才能完成管理决策:
- search: 需要联网搜索最新信息
- ask_user: 需要用户澄清意图或补充背景
- memory: 需要查询外部持久记忆 (如之前会话的关键决策)
- local_file: 需要读取本地文件内容
- none: 当前信息足够, 无需补充
输出字段: `knowledge_gaps`
